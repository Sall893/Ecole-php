...
?>
<center><h5>Ajouter <?=$quest_type?> Question</h5></center>
...
<form method="post" enctype="multipart/form-data">
	...
	<div class="input-group mb-3 pt-3">
	  <label class="input-group-text" for="inputGroupFile01">Commentaire(optionel)</label>
	  <input type="text" name="comment" value="<?=get_var('comment')?>" class="form-control" placeholder="Comment">
	</div>
	
	<div class="input-group mb-3 ">
	  <label class="input-group-text" for="inputGroupFile01"><i class="fa fa-image"></i>image(optionelle)</label>
	  <input type="file" name="image" class="form-control" id="inputGroupFile01">
	</div>
	
	<?php if(isset($_GET['type']) && $_GET['type'] == "objective"):?>
		<div class="input-group mb-3 ">
		  <label class="input-group-text" for="inputGroupFile01">Reponse</label>
	...
	<?php if(isset($_GET['type']) && $_GET['type'] == "multiple"):?>
		<div class="card" style="">
		  <div class="card-header bg-secondary text-white">
		    Question a reponse Multiples <button onclick="add_choice()" type="button" class="btn btn-warning btn-sm float-end"><i class="fa fa-plus"></i>Ajouter Choix</button>
		  </div>
		  <ul class="list-group list-group-flush choice-list">
		            ...
		            if(strstr($key, 'choice')){
 		                ?>
		                    <li class="list-group-item">
						    	...
						    	<label style="cursor: pointer;"><input type="radio" <?= $letters[$num] == $_POST['correct_answer'] ? 'checked' : '';?> value="<?=$letters[$num]?>" name="correct_answer"> Reponse Correcte</label>
						    </li>
						<?php 
 		                $num++;
		            }
			...
			<?php else:?>
			    <li class="list-group-item">
			    	...
			    	<label style="cursor: pointer;"><input type="radio" value="A" name="correct_answer"> Reponse Crorrecte</label>
			    </li>

			    <li class="list-group-item">
			    	B : <input type="text" class="form-control" name="choice1" placeholder="Type your answer here">
			    	<label style="cursor: pointer;"><input type="radio" value="B" name="correct_answer"> Reponse Correcte</label>
			    </li>
			<?php endif;?>
	...
	<a href="<?=ROOT?>/single_test/<?=$row->test_id?>">
		<button type="button" class="btn btn-primary"><i class="fa fa-chevron-left"></i>Reour</button>
	</a>

	<button class="btn btn-danger float-end">Sauvegarder Questions</button>
	<div></div>
</form>
