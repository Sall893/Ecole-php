...
<div class="container-fluid text-center">
	<div class="text-danger"><?=$percentage?>% deja Repondu</div>
	<div class="bg-primary" style="width: <?=$percentage?>%;height: 5px;"></div>

	<div class="text-danger"><?=$marked_percentage?>% déja Marqué</div>
	...
	<?php if($answered_test_row):?>
		<?php if($answered_test_row->submitted && !$marked):?>
			...
			<a onclick="set_test_as_marked(event)" href="<?=ROOT?>/mark_test/<?=$row->test_id?>/<?=$answered_test_row->user_id?>/?set_marked=true">
				<button class="btn mx-1 btn-secondary float-end">Definif comme marqué</button>
			</a>
			
			<a onclick="auto_mark(event)" href="<?=ROOT?>/mark_test/<?=$row->test_id?>/<?=$answered_test_row->user_id?>/?auto_mark=true">
				<button class="btn mx-1 btn-warning float-end">Auto marquage</button>
			</a>
...
<nav class="navbar">
	<center>
		<h5>Questions Devoir</h5>
		<p><b>Questions Totales:</b> <?=$total_questions?></p>
	</center>
