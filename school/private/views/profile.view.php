	...
	<div class="container-fluid p-4 shadow mx-auto" style="max-width: 1000px;">
		...
		<div class="row">
			...
			<div class="col-sm-8 col-md-9 bg-light p-2">
				<table class="table table-hover table-striped table-bordered">
					<tr><th>Prenom:</th><td><?=esc($row->firstname)?></td></tr>
					<tr><th>Nom:</th><td><?=esc($row->lastname)?></td></tr>
					<tr><th>Email:</th><td><?=esc($row->email)?></td></tr>
					<tr><th>Genre:</th><td><?=esc($row->gender)?></td></tr>
					<tr><th>Rang:</th><td><?=ucwords(str_replace("_"," ",$row->rank))?></td></tr>
					<tr><th>Date De Creation:</th><td><?=get_date($row->date)?></td></tr>
		...
		<div class="container-fluid">
			<ul class="nav nav-tabs">
			  <li class="nav-item">
			    <a class="nav-link <?=$page_tab=='info' ? 'active':'';?>" href="<?=ROOT?>/profile/<?=$row->user_id?>">Info De Base</a>
			  </li>
			  <?php if(Auth::access('lecturer') || Auth::i_own_content($row)):?>
				  <li class="nav-item">
				    <a class="nav-link <?=$page_tab=='classes' ? 'active':'';?>" href="<?=ROOT?>/profile/<?=$row->user_id?>?tab=classes">Mes Classes</a>
				  </li>

				  <li class="nav-item">
				    <a class="nav-link <?=$page_tab=='tests' ? 'active':'';?>" href="<?=ROOT?>/profile/<?=$row->user_id?>?tab=tests">Devoir</a>
				  </li>
		 	<?php endif;?>
			</ul>
		...
		<?php else:?>
			<center><h4>Profile non trouvé!</h4></center>
		<?php endif;?>
